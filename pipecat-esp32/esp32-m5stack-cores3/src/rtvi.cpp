...
static const char *get_string_value(cJSON *object, const char *key) {
...
static const char *get_qr_code_text(cJSON *msg) {
  static const char *keys[] = {"text", "url", "value", "content"};

  cJSON *j_data = cJSON_GetObjectItem(msg, "data");
  for (size_t i = 0; i < sizeof(keys) / sizeof(keys[0]); i++) {
    const char *value = get_string_value(j_data, keys[i]);
    if (value != NULL) {
      return value;
    }
  }
  ...
  for (size_t i = 0; i < sizeof(keys) / sizeof(keys[0]); i++) {
    const char *value = get_string_value(msg, keys[i]);
    if (value != NULL) {
      return value;
    }
  }
...
static void rtvi_handle_message(const rtvi_msg_t *msg) {
  ...
  switch (hash(j_type->valuestring)) {
    ...
    case hash("show-qr-code"):
    case hash("display-qr-code"):
    case hash("qr-code"): {
      const char *qr_text = get_qr_code_text(msg->msg);
      if (qr_text == NULL) {
        ESP_LOGE(LOG_TAG, "QR code message missing text/url/value/content");
        break;
      }
      pipecat_screen_show_qr_code(qr_text);
      break;
    }
    case hash("hide-qr-code"):
    case hash("clear-qr-code"):
      pipecat_screen_hide_qr_code();
      break;
    default:
      break;
  }
